 ...
 */
    ...
    private function getDataFromMySQL($dbInfo, $query, $currentPage, $perPage)
    {
        ...
        //TODO: Custom table
        /// Để custom câu lệnh sql
        /// Thay $dbInfo['table'] = (Select....) as new_table
        $dbInfo['table'] = "(SELECT movie.movie_id, url_image, movie_name, sum_view, avg_rate from movie
                        left join (select movie_id, sum(view) as sum_view
                            from episode where status = 1 group by movie_id) as t_episode
                        on t_episode.movie_id = movie.movie_id
                        left join (select movie_id, avg(rate) as avg_rate
                            from movie_detail where status =1 group by movie_id) as t_movie_detail
                        on t_movie_detail.movie_id = movie.movie_id
                        where movie.status = 1 group by movie.movie_id) as t";
    ...
    public function renderView($dbInfo, $query, $currentPage, $perPage)
    {
        ...
        //TODO: Chỉnh lại đường dần tới templates/default.php
        $templatePath = realpath(__DIR__ . $DS . '..' . $DS . '..' . $DS . '..' . $DS . '..' . $DS .
                                'public' . $DS . 'frontend' . $DS . 'plugins' . $DS . 'Ajax_live_search' . $DS .
                                'templates' . $DS . $templateName);
