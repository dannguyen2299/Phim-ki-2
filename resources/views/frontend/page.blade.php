                ...
                <div class="col-xl-8 mt-3">
                    <table id="">
                        ...
                        <tr>
                            <td colspan="3">
                                @if ($episode!=null)
                                <a href="../movie/page-movie-{{ $movie_detail->movie_id }}&episode-{{ $episode->episode_id }}&server-{{ 1 }}.html" class="btn btn-danger mt-3">XEM PHIM</a>
                                @else
                                <a class="btn btn-danger mt-3 no-film">XEM PHIM</a>
            ...
            <div class="row-2 pt-4 mt-2">
                ...
                <div class="row">
                    <ul class="list-inline col rating" title="Average Rating">
                        @for ($count = 1; $count <= 10; $count++) @php if ($count <=$rates[$movie_detail->movie_id]) {
                            $color = 'color:#ffcc00;';
                            } else {
                            $color = 'color:#ccc;';
                            }

                            if ($count <= 2) { $tils='Không hay!' ; } elseif ($count> 2 && $count <= 5) { $tils='Bình thường!' ; }elseif ($count> 5 && $count <= 8) { $tils='Hay!' ; } else { $tils='Rất hay' ; } @endphp <li title="{{$tils}}" id="{{ $movie_detail->movie_id }}-{{ $count }}" data-index="{{ $count }}" data-product_id="{{ $movie_detail->movie_id }}" data-rating="{{ $rates[$movie_detail->movie_id] }}" class="ml-2 ratess" style="cursor:pointer; display: inline-block; {{ $color }}; font-size:25px; ">
                                        &#9733;
                                        </li>
